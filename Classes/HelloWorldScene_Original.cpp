...
/*	Vector<SpriteFrame*> uccelloFrames(4);
	char str[100] = {0};
	for(int i = 1; i < 5; i++)
	{
		//sprintf(str, "Images/grossini_dance_%02d.png",i);
		sprintf(str, "nemico_%02d.png",i);
		auto frame = SpriteFrame::create(str,Rect(0,0,120,84));  //we assume that the sprites' dimentions are 40*40 rectangles.
	     uccelloFrames.pushBack(frame);
	}

	auto animationUcc = Animation::createWithSpriteFrames(uccelloFrames, 0.2f);

	_uccello->runAction( RepeatForever::create( Animate::create(animationUcc) ) );
*/
  ...
  //  NOMI_SFONDO_VECTOR.push_back("montagna_5.png");
    ...
    _montagna = Sprite::create(NOMI_SFONDO_VECTOR[0]);
    _montagna->setAnchorPoint(Vec2(0.0,  0.0));
    _montagna->setPosition(Vec2(origin.x,  origin.y+(_prato->getContentSize().height * .70)));
	this->addChild(_montagna, 4);


	  if(screenSize.width>_montagna->getContentSize().width){
		  _montagna->setScaleX((screenSize.width/ _montagna->getContentSize().width));
	  }
...
Sprite* HelloWorld::createEplosioneAnimata(){
    ...
    for(int i = 1; i < 6; i++)
    {
        ...
        auto frame = SpriteFrame::create(str,Rect(0,0,width_esplosione,heigth_esplosione)); //we assume that the sprites' dimentions are 40*40 rectangles.
        animFrames.pushBack(frame);
    }
...
void HelloWorld::setupActions()
{
    ...
    for (int i = 1; i <3; i++) {
        sprintf(str, "gugut_%02d.png",i);
        auto frame1 = SpriteFrame::create(str,Rect(0,0,width_gugut,heigth_gugut));  //we assume that the sprites' dimentions are 40*40 rectangles.
    ...
    for(int i = 3; i < 7; i++)
    {
        ...
        auto frame = SpriteFrame::create(str2,Rect(0,0,width_gugutsaluta,heigth_gugutsaluta));  //we assume that the sprites' dimentions are 40*40 rectangles.
    ...
    // hit meteor or health animation
	   ...
	   for (int i = 1; i < 7; i++) {
           name.str("");
           name << "bomba_0" << i << ".png";
           auto frame = SpriteFrame::create(name.str(),Rect(0,0,width_esplosione,heigth_esplosione));
