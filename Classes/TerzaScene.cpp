...
//fonts/futura-48.fnt
const std::string FONT_CONTATORI = "font-issue1343.fnt";

//impostazione grafica sfondo
std::string PNG_MONTAGNA = "montagna.png";
std::string PNG_MONTAGNA_GIALLA = "montagna_gialla.png";
std::string PNG_PRATO = "prato.png";
std::string PNG_MURO = "muro.png";
std::string PNG_PALLINA_LABEL = "pietralabel.png";
...
Scene* TerzaScene::createScene()
{
    // 'scene' is an autorelease object
    auto scene = Scene::create();

    // 'layer' is an autorelease object
    auto layer = TerzaScene::create();
...
//aggiunto gufo
    ...
    auto closeItem = MenuItemImage::create(
                                           "CloseNormal.png",
                                           "CloseSelected.png",
                                           CC_CALLBACK_1(TerzaScene::menuCloseCallback, this));
...
*/
  ...
  //  NOMI_SFONDO_VECTOR.push_back("montagna_5.png");
    ...
    _montagna = Sprite::create("sfondoTRE960640.png");
...
#define MUMEROPALLINE 4
    ...
    */
    //agiungo altre palline
      ...
      {
        	auto *pallina = PallinaSprite::pallinaSpriteWithFile("tre_bolla_celeste.png");
        	pallina->setPunti(10);
       		 pallina->setVisible(false);
        	pallina->setTag(2);
        	this->addChild(pallina, 9);
        ...
        {
        	auto *pallina = PallinaSprite::pallinaSpriteWithFile("tre_bolla_viola.png");
        	pallina->setPunti(10);
       		 pallina->setVisible(false);
        	pallina->setTag(3);
        	this->addChild(pallina, 9);
   ...
   // ******************************   HUD   ***********************************
    ...
    //TODO mettero pietra
    _labelLiv= Label::createWithBMFont(FONT_CONTATORI, "Lives");
    addChild(_labelLiv);
    ...
    //appare in alto a sinistra ********* PALLINA *****
     _pietraLabel =  Sprite::create(PNG_PALLINA_LABEL);
    ...
    auto per= Label::createWithBMFont(FONT_CONTATORI, "X");
    addChild(per,23);
    ...
    _livesDisplay = Label::createWithBMFont(FONT_CONTATORI, "0");
    ...
    //  Va posizionata rispetto a LivesDisplay
     auto labelSco = Label::createWithBMFont(FONT_CONTATORI, "Score");
    ...
    //  ****************   SCORE **********************
    _scoreDisplay = Label::createWithBMFont(FONT_CONTATORI, "0");
    ...
    //create main loop
    this->schedule(schedule_selector(TerzaScene::update));

    this->schedule(schedule_selector(TerzaScene::addPalline), 2.4f);
    this->schedule(schedule_selector(TerzaScene::addUccello), 5.0f);
    this->schedule(schedule_selector(TerzaScene::addGugo), 7.0f);
    this->schedule(schedule_selector(TerzaScene::addPallineZigZag), 9.0f);


    auto dispatcher = Director::getInstance()->getEventDispatcher();
    auto listener = EventListenerTouchOneByOne::create();

    listener->onTouchBegan = CC_CALLBACK_2(TerzaScene::onTouchBegan, this);
    //listener->onTouchMoved = CC_CALLBACK_2(TerzaScene::onTouchMoved, this);
...
void TerzaScene::addGugo(float dt){
    if (!_playing) return;
...
void TerzaScene::addUccello (float dt){
   if (!_playing) return;
   if (!(_statoDifficolta == LIVELLO4 || _statoDifficolta == LIVELLO6 ) ){
            ...
            cocos2d::log("TerzaScene::addUccello fine.....");
   }
...
void TerzaScene::addPallineZigZag (float dt){
    cocos2d::log("TerzaScene::addPallineZigZag  .....");
    Size visibleSize = Director::getInstance()->getVisibleSize();
    Vec2 origin = Director::getInstance()->getVisibleOrigin();
...
void TerzaScene::addPalline (float dt){
    cocos2d::log("TerzaScene::addPalline .....");
    if (!_playing) return;
   ...
   // auto seq2 = Sequence::create(moveToTargetPosition, NULL);
    ...
    pallina->runAction(
                       Sequence::create(moveToTargetPosition,
                                        CallFuncN::create(CC_CALLBACK_1(TerzaScene::setInvisible, this)),
...
void TerzaScene::setInvisible(Node * node) {
    node->setVisible(false);
}


bool  TerzaScene::onTouchBegan(Touch *touch, Event *unused_event){
...
void TerzaScene::update (float dt) {
    //the main loop
    if (!_playing) return;
    cocos2d::log("TerzaScene::update .....");
...
//void TerzaScene::stopGameAndMessageOLd()
...
/*
 ...
 sprite->setVisible(false);
	...
	auto restartItem = MenuItemLabel::create(restartLabel, CC_CALLBACK_1(TerzaScene::restartTapped, this));
...
//stoppo il game e mostro messaggio
/*
 void TerzaScene::stopGameAndMessage()
 {
 Size visibleSize = Director::getInstance()->getVisibleSize();
 Vec2 origin = Director::getInstance()->getVisibleOrigin();
 _playing = false;
 ...
 //	"gameoverbutton.png");
	...
	CC_CALLBACK_1(TerzaScene::restartTapped, this));
...
void TerzaScene::restartTapped(Ref* pSender) {
...
Sprite* TerzaScene::createEplosioneAnimata(){
...
void TerzaScene::setupActions()
{
    ...
    _groundHit = Sequence::create(
                                  ...
                                  CallFuncN::create(CC_CALLBACK_1(TerzaScene::animationDone, this)),
                                  NULL);
    _groundHit->retain();
...
void TerzaScene::setupTest2(TerzaScene *seStesso){
...
void TerzaScene::ruotaGugutSinistra(PallinaSprite* pallinaSprite){
...
void TerzaScene::andLivesGugutSalta(){
	_statoGgugut = RUOTASINISTRA;
	   Size visibleSize = Director::getInstance()->getVisibleSize();
	  Vec2 origin = Director::getInstance()->getVisibleOrigin();
	  _gugutAnimato->stopAllActions();
...
void TerzaScene::gugutSaluta(PallinaSprite* pallinaSprite){
   	_statoGgugut = RUOTASINISTRA;
	   Size visibleSize = Director::getInstance()->getVisibleSize();
	  Vec2 origin = Director::getInstance()->getVisibleOrigin();
	  _gugutAnimato->stopAllActions();
...
void TerzaScene::muoviMontagnaScoglio(){
...
void TerzaScene::animationDone(Node *node)
{
    node->stopAllActions();
}
void TerzaScene::effettoGuadagnoPietre(Node *node)
{
...
void TerzaScene::controllaModificaDifficolta()
{
...
void TerzaScene::controllaPietre()
{
...
float TerzaScene::randomValueBetween(float low, float high)
{
    return (((float) random() / 0xFFFFFFFFu) * (high - low)) + low;
}
void TerzaScene::menuCloseCallback(Ref* pSender)
{
...
void TerzaScene::particelleRotFlower()
{
...
void TerzaScene::particellePallinaNegativa()
{
...
void TerzaScene::controllaPietraConPallinaBlu(Node *pietra)
{
